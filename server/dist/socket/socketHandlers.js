...
function setupSocketHandlers(io, roomManager) {
    ...
    function emitGameStateToAll(roomCode) {
    ...
    function emitNightPhaseToAll(roomCode) {
        const room = roomManager.getRoom(roomCode);
        if (!room)
            return;
        for (const playerId of room.getConnectedPlayerIds()) {
    ...
    function emitVotingToAll(roomCode) {
        const room = roomManager.getRoom(roomCode);
        if (!room)
            return;
        emitToRoom(roomCode, 'votingUpdate', room.getVotingData());
    }
    function emitNarration(roomCode, narrations) {
        emitToRoom(roomCode, 'narration', narrations);
    }
    function broadcastAndState(roomCode, narrations) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('createRoom', (data, callback) => {
            ...
            logger_1.logger.gameEvent('Room created', result.roomCode, { host: playerName });
        });
        socket.on('joinRoom', (data, callback) => {
            ...
            logger_1.logger.gameEvent('Player joined', currentRoomCode, { name: playerName });
        });
            ...
            for (const code of roomManager.getRoomCodes()) {
                ...
                if (player) {
                    ...
                    if (phase.startsWith('night_')) {
                        ...
                        if (nightData)
                            socket.emit('nightPhaseUpdate', nightData);
                    }
                    else if (phase === 'day_voting') {
                        socket.emit('votingUpdate', room.getVotingData());
                    }
                    logger_1.logger.gameEvent('Player rejoined', code, { name: player.name });
                    return;
                }
            }
        ...
        socket.on('startGame', (callback) => {
            ...
            if (!narrations)
                return callback({ success: false, error: 'Cannot start. Need at least 5 players.' });
        ...
        socket.on('readyForNight', () => {
            if (!currentRoomCode)
                return;
            const room = roomManager.getRoom(currentRoomCode);
            if (!room || room.getPhase() !== 'role_reveal')
                return;
            const player = room.getPlayer(currentPlayerId);
            if (!player?.isHost)
                return;
        ...
        socket.on('startVoting', () => {
            if (!currentRoomCode || !currentPlayerId)
                return;
            const room = roomManager.getRoom(currentRoomCode);
            if (!room || room.getPhase() !== 'day_discussion')
                return;
            const player = room.getPlayer(currentPlayerId);
            if (!player?.isHost)
                return;
        ...
        socket.on('startNextNight', () => {
            ...
            if (!player?.isHost)
                return;
            if (room.getPhase() !== 'day_discussion')
                return;
            const narrations = room.startNight();
            broadcastAndState(currentRoomCode, narrations);
        });
        // --- NIGHT ACTIONS ---
        socket.on('nightSelect', (data) => {
            if (!currentRoomCode || !currentPlayerId)
                return;
            const room = roomManager.getRoom(currentRoomCode);
            if (!room)
        ...
        socket.on('disconnect', () => {
            ...
            logger_1.logger.info(`Client disconnected: ${socket.id} (${playerName})`);
